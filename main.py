...
from filament_common import DrawFilament
import numpy as np
import time
...
for fil_i in range(6, 7):
# for fil_i in range(1, 10):  # test with filament1
    if fil_i not in fil_pool.keys():
        continue

    fila = DrawFilament(fila_n=fil_i,min_l=fil_pool[fil_i][0], max_l=fil_pool[fil_i][1],
                                x_box=fil_pool[fil_i][2], y_box=fil_pool[fil_i][3],
                                line_b=fil_pool[fil_i][4])
    try:
        fila.output_graph(threshold=np.linspace(0.04, 0.09, 6))

    except ZeroDivisionError:
        error_log.write('filament%d got an error!' % (fil_i))

    # fila.draw_tree(threshold=np.linspace(0.04, 0.09, 1))
    # get bg figure draw
    # fil_candidate_file = '/Users/penny/Works/MST_filaments/' \
    #                      'Extinction_filaments_data/Candid%d_cropped_final.fits' % fil_i
    # bg_fig = get_bg_figure(fil_candidate_file)
    # for i in np.linspace(0.01, 0.2, 20):  # threshold
    # for i in np.linspace(0.04, 0.09, 1):  # threshold
    #     try:
    #         start = time.time()
