...
class DrawAllData():
    ...
    def __init__(self, lon_range, threshold):  # lon_range identical to file name
        self.lat = []
        self.lng = []

        # the bg image lon range
        self.max_l = lon_range+1.5
        self.min_l = lon_range-1.5
        ...
        # init the graph
        self.create_graph()
        # TODO: think about three digit number
        self.fil_candidate_file = '/Users/penny/Works/MST_filaments/' \
                                  'Glimpse_GLON_10-60/GLM_0%d00+0000_mosaic_I3.fits' % self.lon_range
        if not os.path.exists('./GLM_0%d_output' % self.lon_range):
            os.makedirs('./GLM_0%d_output' % self.lon_range)
    ...
    def get_bg_figure(self):
        ...
        # TODO: those show filament should be moved to a layer
    ...
    def save_tree(self):
        i = 1
        workbook = Workbook()
        ws = workbook.add_sheet('GLM_0'+str(self.lon_range))
        ...
        start = time.time()
        fig_name = './GLM_0%d_output/GLM_0%d_%.2f_MST.png' % (self.lon_range, self.lon_range, self.threshold)
        plt.savefig(fig_name)

        end = time.time()
        print('save fig time', end - start)  # TODO: here is 10 sec, almost half of time

        wb_name = './GLM_0%d_output/tree1_%.2f.xls' % (self.lon_range, self.threshold)
